...
#            print("\t\ti={} {}".format(i,seqwrd2))
            ...
            for k in range(len(seq2)):
                if seq2[k] == seq[k]: 
#                    print("not totally different")
                    totally_different = False
                    break
            if totally_different: return seq2
...
class dataset():
    ...
    def add(self,src,tgt,ali,pos):
        if len(src): self.SRC.append(src)
        if len(tgt): self.TGT.append(tgt)
        if len(ali): self.ALI.append(ali)
        if len(pos): self.POS.append(pos)
    ...
    def parallel_pair(self,i,st,o):
        ...
        st.n_sents += 1
        if o.debug: self.debug('PARALLEL',src,src_tag,tgt,tgt_tag)
    ...
    def uneven_pair(self,i,st,o):
        ...
        while True:
            ...
            if i%2 == 0:
                ...
                src = list(self.SRC[i])
                tgt = list(self.TGT[j]) ### replace tgt
            if len(src)>len(tgt) and len(src)*0.5>len(tgt): continue 
            if len(tgt)>len(src) and len(tgt)*0.5>len(src): continue 
            break
        ...
        st.n_sents += 1
        if o.debug: self.debug('UNEVEN (ntry={})'.format(n_try),src,src_tag,tgt,tgt_tag)
    ...
    def insert_pair(self,i,st,o):
        ...
        where = ""
        if len(src)<=len(tgt): ### add in src side
            n_try = 0
            while True:
                n_try += 1
                if n_try > self.max_ntry: return ### cannot find the right pair
        ...
        st.n_tgt_words += len(tgt)
        st.n_sents += 1
        if o.debug: self.debug('INSERT {} (ntry={})'.format(where,n_try),src,src_tag,tgt,tgt_tag)
        return
    ...
    def delete_pair(self,i,st,o):
        ...
        while True:
            n_try += 1
            if n_try >= self.max_ntry: return
            imin = np.random.randint(0,len(tgt_orig))
            imax = np.random.randint(imin,len(tgt_orig))
            if imax-imin+1 > len(tgt_orig)//2: continue
            src_from, src_to, tgt_from, tgt_to = self.expand_self_contained_sequence(imin,imax,a)
            if src_from<0 or src_to<src_from or src_to>=len(src_orig): continue
            if tgt_from<0 or tgt_to<tgt_from or tgt_to>=len(tgt_orig): continue
            if src_to-src_from+1 > len(src_orig)//2: continue
            if tgt_to-tgt_from+1 > len(tgt_orig)//2: continue
            break
        ...
        deleted = []
        where = ""
        if len(src_orig)>=len(tgt_orig): ### delete on src side mark on tgt side
            where = "src"
            for s in range(len(src_orig)):
                if s < src_from or s > src_to: ### keep the word
                    src.append(src_orig[s])
                    src_tag.append('-1.0')
                else:
                    deleted.append(src_orig[s])
            for t in range(len(tgt_orig)):
                tgt.append(tgt_orig[t]) ### keep all tgt words
                if t < tgt_from or t > tgt_to: ### mark non-divergent (the aligned src word is kept)
                    tgt_tag.append('-1.0')
                else:
                    tgt_tag.append('1.0') ### mark divergent (the aligned src word is deleted)
                    st.n_tgt_divergent += 1
        else: ### delete on tgt side mark on src side
            where = "tgt"
            for t in range(len(tgt_orig)):
                if t < tgt_from or t > tgt_to: ### keep the word
                    tgt.append(tgt_orig[t])
                    tgt_tag.append('-1.0')
                else:
                    deleted.append(tgt_orig[t])
            for s in range(len(src_orig)):
                src.append(src_orig[s]) ### keep all src words
                if s < src_from or s > src_to: ### mark non-divergent (the aligned tgt word is kept)
                    src_tag.append('-1.0')
                else:
                    src_tag.append('1.0') ### mark divergent (the aligned tgt word is deleted)
        ...
        st.n_tgt_words += len(tgt)
        st.n_sents += 1
        if o.debug: self.debug('DELETE {} [{},{}][{},{}] {} (ntry={})'.format(where,src_from,src_to,tgt_from,tgt_to," ".join(deleted),n_try),src,src_tag,tgt,tgt_tag)
        return
    ...
    def replace_pair(self,i,st,o):
        ...
        st.n_tgt_words += len(tgt)
        st.n_sents += 1
        if o.debug: self.debug('REPLACE [{},{}][{},{}] {} => {}'.format(src_from,src_to,tgt_from,tgt_to,replace_orig,replace_dest),src,src_tag,tgt,tgt_tag)
        return
    ...
    def find_replacement(self,src,tgt,ali,pos):
        ...
        for src_from in indexs:
            ...
            for src_to in range(src_from+self.r.max_length-1,src_from+self.r.min_length-2,-1):
                if src_to>=len(src): continue
                if src_to-src_from+1 > len(src)//2: continue
    ...
    def replacement(self,src,tgt,src_from,src_to,pos,a):
        ...
        if len(src_rep)!=len(src_seq): return -1, -1, []
        ...
        for s in range(src_from, src_to+1):
            ...
            if tgt_min is not None and tgt_min < tgt_from: tgt_from = tgt_min
            if tgt_max is not None and tgt_max > tgt_to:   tgt_to   = tgt_max
        if tgt_from == 9999 or tgt_to == -1: return -1, -1, []
        ...
        for t in range(tgt_from, tgt_to+1):
            ...
            if src_min is not None and src_min < src_from: return -1, -1, []
            if src_max is not None and src_max > src_to:   return -1, -1, []
    ...
    def expand_self_contained_sequence(self, imin, imax, a):
        ...
        #2 : find the minimum sequence of source/target words that contains [jmin,jmax][imin,imax] and are not aligned outside the sequence
        j_or_i = 0
        while True:
            if j_or_i == 0: jmin, jmax = self.MinMax(a.t2s_minmax,imin,imax)
            else:           imin, imax = self.MinMax(a.s2t_minmax,jmin,jmax)
            if jmin is None or jmax is None or imin is None or imax is None: return -1, -1, -1, -1
            if length_sequences == (imax-imin+1) + (jmax-jmin+1): break #no added words stop iterating
        ...
        for line in f:
            ...
            if len(tok)>0:
                ...
                if len(pos)>0 and len(src)!=len(pos):
                    sys.stderr.write('warning: different number of src/pos tokens in line {} [skipping line]\n'.format(i))
                    continue
            d.add(src,tgt,ali,pos)
    t1 = time.time()
    sys.stderr.write('Read {} sentence pairs ({}/{} pruned) time: {:.3f} s\n'.format(n_total,n_src_prune,n_tgt_prune,t1-t0))
            
    st = stats()
    indexs = [i for i in range(len(d))]
    if o.shuffle: shuffle(indexs)
    for i in indexs:
        if 'p' in o.mode: d.parallel_pair(i,st,o)
        if 'u' in o.mode: d.uneven_pair(i,st,o)
        if 'i' in o.mode: d.insert_pair(i,st,o)
        if 'r' in o.mode: d.replace_pair(i,st,o)
        if 'd' in o.mode: d.delete_pair(i,st,o)
        if o.max_sents>0 and s.n_sents>=o.max_sents: break    
    t2 = time.time()
    st.show(t2-t1)
