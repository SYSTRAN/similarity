...
class Align():
    def __init__(self, align):
        ...
        # max_cost_s[0][1][2] means max cost of source word 0 from target word 1 to target word 2
        self.max_cost_s = [[[None for c in range(self.ltgt)] for b in range(self.ltgt)] for a in range(self.lsrc)]
    ...
    def cost_square_max_outside(self, s_ini, s_end, t_ini, t_end):
        c = 0.0
        ### source words
        for s in range(0, s_ini):
            c -= self.max_s(s, 0, self.ltgt-1)
        for s in range(s_end+1, self.lsrc):
            c -= self.max_s(s, 0, self.ltgt-1)
        ### target words
        for t in range(0, t_ini):
            c -= self.max_t(t, 0, self.lsrc-1)
        for t in range(t_end+1, self.ltgt):
            c -= self.max_t(t, 0, self.lsrc-1)
        return c
    ...
    def cost_square_max_inside(self, s_ini, s_end, t_ini, t_end):
        c = 0.0
        ### source words
        for s in range(s_ini, s_end+1):
            c += self.max_s(s, t_ini, t_end)
        ### target words
        for t in range(t_ini, t_end+1):
            c += self.max_t(t, s_ini, s_end)
        return c


class Fix():
    def __init__(self, tau, nbest, use_punct, max_sim):
        # hypotheses must be at least sized of tau
        self.tau = tau
        self.nbest = nbest
        # only hypotheses with a similarity score lower than max_sim are considered to be fixed
    ...
    def print_fix_square(self, src, tgt, align, sim, n_sents):
        ...
        ref_pair = " ".join(s for s in src) + "\t" + " ".join(t for t in tgt)
        ...
        if sim <= self.max_sim:
            ...
            for s_ini in range(0, len(src)):
                for s_end in range(s_ini+min_length-1, len(src)):
                    if not self.bounded_by_punctuation(s_ini, s_end, src):
                        continue
                    for t_ini in range(0, len(tgt)):
                        for t_end in range(t_ini+min_length-1, len(tgt)):
                            if not self.bounded_by_punctuation(t_ini, t_end, tgt):
                                continue
                            c = 0.0
                            if self.inside:
                                c += a.cost_square_max_inside(s_ini, s_end, t_ini, t_end)
                            if self.outside:
                                c += a.cost_square_max_outside(s_ini, s_end, t_ini, t_end)
                            ltgt = tgt[t] for t in range(t_ini, t_end+1)
                            pair = " ".join(src[s] for s in range(s_ini, s_end+1)) + "\t" + " ".join(ltgt)
                            cost[pair] = c
    ...
    def bounded_by_punctuation(self, ini, end, vec):
        if not self.use_punct:
            return True
        if ini > 0 and vec[ini-1] not in string.punctuation:
            ### previous to ini is punctuation
            return False
        if vec[end] not in string.punctuation:
            ### end is punctuation
            return False
        return True
